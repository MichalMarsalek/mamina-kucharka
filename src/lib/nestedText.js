...
export function parseNestedText(input) {
	let parsed = {
		column: 1,
		indent: 0,
		line: 0,
	...
	for (let i = lines.length - 1; i >= 0; i -= 1) {
		if (lines[i].type === ParsedLineType.BLANK || lines[i].type === ParsedLineType.COMMENT) {
			lines.splice(i, 1);
		}
	}
	...
	while (i < lines.length - 1) {
		...
		if (before.type === ParsedLineType.STRING_ITEM && after.type === before.type) {
			if (before.indent < after.indent) {
		...
		if (before.type === ParsedLineType.LIST_ITEM) {
			if (after.type === ParsedLineType.LIST_ITEM && before.indent < after.indent && before.value) {
				throwError(after.startIndex + before.indent, after, 'Unexpected indentation');
			}
			if (before.indent === after.indent && after.type !== ParsedLineType.LIST_ITEM) {
...
function parseInlineList(input, index, result) {
	...
	while (true) {
		...
		if (input[index] === ']') {
			index += 1;
			while (isWhitespace(input[index]) && !isNewlineOrEnd(input[index])) {
				index += 1;
			}
...
function parseInlineString(input, index, prohibited) {
	let value = '';
	while (prohibited.indexOf(input[index]) === -1 && !isNewlineOrEnd(input[index])) {
		value += input[index];
		index += 1;
	}
